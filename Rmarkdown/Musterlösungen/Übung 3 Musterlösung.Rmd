...
cohort.all <- mimic.admissions.raw %>%
    ...
    left_join(cohort.akf, by = "hadm_id") %>%  # ACF-Kohorte hinzufügen
...
creatinine <- mimic.labevents.annotated %>%
    filter(grepl("creatinine", tolower(label)), fluid == "Blood") %>%
    inner_join(cohort.all %>% select(hadm_id, cohort_akf_flag), by = "hadm_id")
head(creatinine)
```
...
# Boxplot der Troponin-Auspräungen gegen den AMI-Kohortenstatus ausgeben
ggplot(data = creatinine, aes(x = label, y = valuenum, fill = cohort_akf_flag)) +
    geom_boxplot()
```
...
# Boxplot der Troponin-Auspräungen gegen den AMI-Kohortenstatus ausgeben
ggplot(data = creatinine %>% filter(valuenum < 100), aes(x = label, y = valuenum, fill = cohort_akf_flag)) +
    geom_boxplot()
```
